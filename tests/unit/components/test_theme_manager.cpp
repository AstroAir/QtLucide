...
void TestThemeManager::initTestCase() {
    ...
    m_testDataDir =
        QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/QtLucideThemeTests";
...
void TestThemeManager::testDefaultTheme() {
    ...
    QVERIFY(currentTheme == ThemeManager::SystemTheme || currentTheme == ThemeManager::LightTheme ||
...
bool TestThemeManager::verifyThemeColors(const ThemeManager::ThemeColors& colors) {
    return !colors.name.isEmpty() && colors.colors.contains(ThemeManager::WindowBackground) &&
