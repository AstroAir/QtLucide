...
void TestThemeManager::testColorRetrieval() {
    ...
    QList<ThemeManager::ColorRole> colorRoles = {
        ThemeManager::WindowBackground, ThemeManager::PrimaryText, ThemeManager::AccentColor,
        ThemeManager::BorderColor, ThemeManager::SelectionBackground};

    for (ThemeManager::ColorRole role : colorRoles) {
        QColor color = m_themeManager->getColor(role);
        QVERIFY2(
            color.isValid(),
            qPrintable(QString("Color for role %1 should be valid").arg(static_cast<int>(role))));
    }
...
void TestThemeManager::testThemeColorConsistency() {
    ...
    QVERIFY2(lightAccent != darkAccent,
             "Light and dark themes should have different accent colors");
...
bool TestThemeManager::verifyThemeColorsValid(ThemeManager* themeManager) {
    ...
    QList<ThemeManager::ColorRole> roles = {ThemeManager::WindowBackground,
                                            ThemeManager::PrimaryText, ThemeManager::AccentColor};
