...
void TestLibraryAvailability::testQtLucideHeadersAvailability() {
    ...
    QVERIFY2(isHeaderAvailable("QtLucide/QtLucideIconPainter.h"),
             "QtLucide icon painter header should be available");
...
void TestLibraryAvailability::testQtLucideIconResources() {
    ...
    for (const QString& iconName : commonIcons) {
        ...
        QVERIFY2(!icon.isNull(),
                 qPrintable(QString("Icon '%1' should be available").arg(iconName)));
    }
...
void TestLibraryAvailability::testMinimumQtVersionRequirement() {
    ...
    QVERIFY2(version.majorVersion() >= 6,
             qPrintable(QString("Qt version should be 6.0 or higher, found: %1").arg(qtVersion)));
...
void TestLibraryAvailability::testPlatformCompatibility() {
    ...
    QVERIFY2(supportedPlatform,
             qPrintable(QString("Platform should be supported, found: %1").arg(platform)));
