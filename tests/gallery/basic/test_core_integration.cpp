...
void TestCoreIntegration::initTestCase()
{
    ...
    m_testDataDir = QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/qtlucide_core_test";
...
void TestCoreIntegration::testIconEnumeration()
{
    ...
    QStringList expectedIcons = {"house", "user", "settings", "search", "heart", "star", "check", "x"};
...
void TestCoreIntegration::testIconRendering()
{
    ...
    for (const QString& iconName : testIcons) {
        for (int size : testSizes) {
            ...
            QVERIFY2(!pixmap.isNull(), qPrintable(QString("Failed to render pixmap for %1 at size %2").arg(iconName).arg(size)));
            ...
            QVERIFY2(verifyIconRenderingQuality(iconName, size), 
                    qPrintable(QString("Rendering quality check failed for %1 at size %2").arg(iconName).arg(size)));
        }
    }
    
    qDebug() << "Icon rendering test passed for" << testIcons.size() << "icons at" << testSizes.size() << "sizes";
}
...
void TestCoreIntegration::testIconCaching()
{
    ...
    qDebug() << "Icon caching test completed - First load:" << firstLoadTime << "ms, Second load:" << secondLoadTime << "ms";
}
...
void TestCoreIntegration::testResourceManagement()
{
    ...
    for (int i = 0; i < instanceCount; ++i) {
        ...
        QVERIFY2(instance->initLucide(), qPrintable(QString("Failed to initialize instance %1").arg(i)));
...
void TestCoreIntegration::testErrorHandling()
{
    ...
    for (const QString& invalidIcon : invalidIcons) {
        ...
        QVERIFY2(icon.isNull(), qPrintable(QString("Expected null icon for invalid name: %1").arg(invalidIcon)));
    }
