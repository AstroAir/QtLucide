...
class IconUsageValidator:
    ...
    def load_available_icons(self) -> bool:
        ...
        metadata_file = self.project_root / "resources" / "icons" / "metadata" / "icons.json"
    ...
    def _load_from_metadata(self, metadata_file: Path) -> bool:
        ...
        try:
            ...
            if isinstance(metadata, dict):
                ...
                self.available_icons = set(item.get('name', '') for item in metadata if 'name' in item)
    ...
    def _looks_like_icon_name(self, name: str) -> bool:
        ...
        non_icon_words = {
            ...
            'error', 'warning', 'info', 'debug', 'test', 'example', 'sample',
            'main', 'window', 'widget', 'button', 'label', 'layout', 'dialog',
            'application', 'version', 'author', 'license', 'copyright',
            'true', 'false', 'null', 'undefined', 'none', 'empty',

            # UI/CSS terms
            'color', 'size', 'width', 'height', 'opacity', 'scale-factor',
            'geometry', 'icon', 'svg', 'png', 'ico', 'pdf', 'icns',
            'selected', 'hovered', 'favorite', 'name', 'format', 'mode',
            'clicked', 'about', 'value', 'count', 'icons', 'px', 'ms',

            # Theme/appearance terms
            'theme', 'system', 'light', 'dark', 'compact', 'grid',
            'language', 'en', 'black', 'white',

            # Test-specific terms
            'non-existent-icon', 'test-icon', 'invalid-icon', 'example-icon',
            'heart-icon', 'icon-heart', 'test-null', 'not-a-color',
            'not-a-number', 'corrupted-test', 'null-painter', 'invalid-painter',
            'exception-painter', 'test-painter', 'memory-test-painter',
            'replacement-painter', 'lifetime-painter', 'non-existent',
            'invalid', 'unknown', 'high-contrast', 'color-disabled',
            'test-option', 'test-value', 'race-painter', 'extreme-painter',
            'memory-painter', 'slow-painter',

            # Data/metadata terms
            'categories', 'contributors', 'favorites', 'timestamp', 'usage',
            'recent', 'criteria', 'quotes', 'with', 'nested', 'level1',
            'level2', 'level3', 'value1', 'value2', 'inner', 'x64',

            # Generic terms
            'arrows', 'communication', 'media', 'cal', 'arrow'
        }
    ...
    def scan_directory(self, directory: Path, extensions: Optional[List[str]] = None) -> None:
