...
public:
    ImageGalleryTestWindow(QWidget* parent = nullptr)
        : QMainWindow(parent), m_lucide(new lucide::QtLucide(this)),
          m_imageMetadataManager(new ImageMetadataManager(this)),
          m_contentManager(new ContentManager(this)), m_imageViewer(new ImageViewerWidget(this)),
...
private:
    void setupUI() {
        ...
        connect(openDirButton, &QPushButton::clicked, this,
                &ImageGalleryTestWindow::onOpenDirectory);
        connect(testIconsButton, &QPushButton::clicked, this, [this]() {
            ...
            if (m_lucide->initLucide()) {
                ...
                m_statusLabel->setText(
                    QString("QtLucide initialized with %1 icons").arg(icons.size()));
            } else {
    ...
    void setupConnections() {
        ...
        connect(m_fileBrowser, &FileBrowserWidget::imageSelected, this,
                &ImageGalleryTestWindow::onImageSelected);
        connect(m_fileBrowser, &FileBrowserWidget::directoryChanged, this,
                &ImageGalleryTestWindow::onDirectoryChanged);

        // Connect content manager signals
        connect(m_contentManager, &ContentManager::contentLoaded, this,
                [this](const QString& identifier, ContentType type) {
                    QString typeStr = (type == ContentType::Icon) ? "icon" : "image";
                    qDebug() << "Content loaded:" << identifier << "(" << typeStr << ")";
                });

        connect(m_contentManager, &ContentManager::contentCountChanged, this,
                [this](int total, int icons, int images) {
