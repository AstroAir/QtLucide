    ...
    , m_suggestionLimit(DEFAULT_SUGGESTION_LIMIT)
{
    ...
    m_suggestionTimer->setInterval(SUGGESTION_DELAY);
    
    connect(this, &QLineEdit::textChanged, this, &SearchLineEdit::onTextChanged);
    connect(m_suggestionTimer, &QTimer::timeout, this, &SearchLineEdit::updateSuggestions);
}
...
void AdvancedSearchPanel::setupUI()
{
    ...
    m_layout->addWidget(m_categoryGroup);
    m_layout->addWidget(m_tagGroup);
    m_layout->addWidget(m_sortGroup);
    m_layout->addWidget(m_optionsGroup);
...
void SearchWidget::setupUI()
{
    ...
    m_layout->addWidget(m_advancedPanel);
}
...
// QuickFilterWidget implementation
QuickFilterWidget::QuickFilterWidget(IconMetadataManager* metadataManager, QWidget *parent)
    : QWidget(parent), m_metadataManager(metadataManager) {
    setupUI();
}
void QuickFilterWidget::updateFilters() { createFilterButtons(); }
void QuickFilterWidget::onFilterButtonClicked() { /* TODO */ }
void QuickFilterWidget::onShowAllClicked() { emit showAllIcons(); }
void QuickFilterWidget::setupUI() {
    m_layout = new QHBoxLayout(this);
    m_buttonGroup = new QButtonGroup(this);
    m_showAllButton = new QToolButton(this);
    m_showAllButton->setText("All");
    m_layout->addWidget(m_showAllButton);
    connect(m_showAllButton, &QToolButton::clicked, this, &QuickFilterWidget::onShowAllClicked);
}
void QuickFilterWidget::createFilterButtons() { /* TODO */ }
