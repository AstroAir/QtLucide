...
ImageFileSystemModel::ImageFileSystemModel(QObject* parent)
    : QFileSystemModel(parent), m_metadataManager(nullptr), m_showOnlyImages(false) {
...
ThumbnailDelegate::ThumbnailDelegate(QObject* parent)
    : QStyledItemDelegate(parent), m_thumbnailSize(64, 64), m_contentManager(nullptr) {}
...
void ThumbnailDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    if (fileInfo.isFile() && m_contentManager) {
        ...
        if (!thumbnail.isNull()) {
            ...
            QRect thumbnailRect = QRect(rect.left() + 4, rect.top() + 4, m_thumbnailSize.width(),
...
FileBrowserWidget::FileBrowserWidget(QWidget* parent)
    : QWidget(parent), m_contentManager(nullptr), m_metadataManager(nullptr),
      m_fileSystemModel(new ImageFileSystemModel(this)),
      m_proxyModel(new QSortFilterProxyModel(this)), m_directoryTree(new QTreeView(this)),
      m_fileList(new QListView(this)), m_thumbnailDelegate(new ThumbnailDelegate(this)),
      m_viewMode(ListView), m_thumbnailSize(128, 128), m_showOnlyImages(true),
      m_showHiddenFiles(false), m_historyIndex(-1),
      m_thumbnailWatcher(new QFutureWatcher<void>(this)), m_updateTimer(new QTimer(this)) {
...
void FileBrowserWidget::setupToolbar() {
    ...
    connect(m_viewModeCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &FileBrowserWidget::onViewModeChanged);
    connect(m_filterCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [this](int index) { setShowOnlyImages(index == 1); });
    connect(m_nameFilterEdit, &QLineEdit::textChanged, this,
            &FileBrowserWidget::onNameFilterChanged);
}
...
void FileBrowserWidget::setupFileList() {
    ...
    connect(m_fileList, &QListView::doubleClicked, this,
            &FileBrowserWidget::onFileListDoubleClicked);
}
...
void FileBrowserWidget::onFileListClicked(const QModelIndex& index) {
    ...
    if (fileInfo.isFile()) {
        ...
        if (m_metadataManager &&
            m_metadataManager->getSupportedFormats().contains(fileInfo.suffix().toLower())) {
...
DirectoryScanner::DirectoryScanner(QObject* parent) : QObject(parent), m_cancelled(false) {}
...
void DirectoryScanner::doScan() {
    ...
    QTimer::singleShot(100, this, [this, totalFiles]() { emit scanFinished(totalFiles); });
}
