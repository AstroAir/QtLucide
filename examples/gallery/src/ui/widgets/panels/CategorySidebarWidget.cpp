...
CategoryItem::CategoryItem(const QString& categoryName, const QString& displayName, int iconCount,
                           QWidget* parent)
    : QWidget(parent), m_categoryName(categoryName), m_displayName(displayName),
      m_iconCount(iconCount), m_selected(false), m_hovered(false) {
    ...
    m_countLabel->setStyleSheet("QLabel {"
    ...
    m_shortcutLabel->setStyleSheet("QLabel {"
                                   "  color: #95a5a6;"
                                   "  font-size: 10px;"
...
CategorySidebarWidget::CategorySidebarWidget(QWidget* parent)
    : QWidget(parent), m_iconMetadataManager(nullptr), m_favoritesManager(nullptr),
      m_currentCategory("all"), m_compactMode(false) {
    ...
    m_categoryInfos = {
        {"business", "Business & Finance", "Charts, finance, and business-related icons", "Ctrl+1",
         0},
...
void CategorySidebarWidget::setFavoritesManager(FavoritesManager* manager) {
    ...
    if (manager) {
        connect(manager, &FavoritesManager::favoritesChanged, this, [this]() {
...
void CategorySidebarWidget::refreshCategories() {
    ...
    if (m_allCategoriesItem) {
        m_allCategoriesItem->setIconCount(static_cast<int>(allIcons.size()));
    }
    ...
    if (m_favoritesManager && m_recentItem) {
        m_recentItem->setIconCount(
            static_cast<int>(m_favoritesManager->getRecentlyViewed(50).size()));
    }
}
...
void CategorySidebarWidget::setupSpecialCategories() {
    ...
    m_specialGroup->setStyleSheet("QGroupBox {"
                                  "  font-weight: 600;"
                                  "  font-size: 12px;"
                                  "  color: #34495e;"
...
void CategorySidebarWidget::setupMainCategories() {
    ...
    m_mainGroup->setStyleSheet("QGroupBox {"
                               "  font-weight: 600;"
                               "  font-size: 12px;"
                               "  color: #34495e;"
                               "  border: 2px solid #ecf0f1;"
    ...
    for (int i = 0; i < m_categoryInfos.size(); ++i) {
        ...
        connect(item, &CategoryItem::contextMenuRequested, this,
                &CategorySidebarWidget::onCategoryContextMenu);
...
void CategorySidebarWidget::updateCategoryCounts() {
    ...
    for (int i = 0; i < m_categoryInfos.size(); ++i) {
        ...
        info.iconCount = static_cast<int>(categoryIcons.size());
...
void CategorySidebarWidget::selectCategory(const QString& category) {
    // Deselect all items
    if (m_allCategoriesItem)
        m_allCategoriesItem->setSelected(false);
    if (m_favoritesItem)
        m_favoritesItem->setSelected(false);
    if (m_recentItem)
        m_recentItem->setSelected(false);
    ...
    if (category == "all") {
        if (m_allCategoriesItem)
            m_allCategoriesItem->setSelected(true);
    } else if (category == "favorites") {
        if (m_favoritesItem)
            m_favoritesItem->setSelected(true);
    } else if (category == "recent") {
        if (m_recentItem)
            m_recentItem->setSelected(true);
...
void CategorySidebarWidget::onCategoryContextMenu(const QString& category,
                                                  const QPoint& globalPos) {
    QMenu menu;
    menu.addAction("Clear Category", [this, category]() { emit clearCategoryRequested(category); });
