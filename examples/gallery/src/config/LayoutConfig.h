...
public:
    ...
    static int getThumbnailSize(ScreenSize screenSize = ScreenSize::Medium) {
        switch (screenSize) {
            case ScreenSize::Small:
                return Sizing::THUMBNAIL_SMALL;
            case ScreenSize::Medium:
                return Sizing::THUMBNAIL_MEDIUM;
            case ScreenSize::Large:
                return Sizing::THUMBNAIL_LARGE;
            case ScreenSize::XLarge:
                return Sizing::THUMBNAIL_XLARGE;
        }
    ...
    static int getSidebarWidth(ScreenSize screenSize = ScreenSize::Medium) {
        switch (screenSize) {
            case ScreenSize::Small:
                return Sizing::SIDEBAR_MIN_WIDTH;
            case ScreenSize::Medium:
                return Sizing::SIDEBAR_PREFERRED_WIDTH;
            case ScreenSize::Large:
                return Sizing::SIDEBAR_PREFERRED_WIDTH + 40;
            case ScreenSize::XLarge:
                return Sizing::SIDEBAR_MAX_WIDTH;
        }
        return Sizing::SIDEBAR_PREFERRED_WIDTH;
    }
    ...
    static int calculateGridColumns(int availableWidth, int thumbnailSize,
        ...
        if (availableWidth <= 0 || thumbnailSize <= 0)
            return 1;
    ...
    static QList<int> getSplitterProportions(ScreenSize screenSize, bool hasThreePanes = false) {
        if (hasThreePanes) {
            // Sidebar : Content : Details
            switch (screenSize) {
                case ScreenSize::Small:
                    return {1, 3, 1}; // Compact layout
                case ScreenSize::Medium:
                    return {1, 4, 1}; // Balanced
                case ScreenSize::Large:
                    return {1, 5, 2}; // More content space
                case ScreenSize::XLarge:
                    return {1, 6, 2}; // Maximum content
            }
        } else {
            // Sidebar : Content
            switch (screenSize) {
