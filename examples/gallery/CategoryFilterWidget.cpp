...
CategoryTreeItem::CategoryTreeItem(QTreeWidget *parent, const QString& category, int iconCount)
    : QTreeWidgetItem(parent, CategoryItem)
...
CategoryTreeItem::CategoryTreeItem(CategoryTreeItem *parent, const QString& tag, int iconCount)
    : QTreeWidgetItem(parent, TagItem)
    , m_categoryName(tag)
    , m_iconCount(iconCount)
{
    updateDisplay();
...
{
    QString text = QString("%1 (%2)").arg(m_categoryName).arg(m_iconCount);
    setText(0, text);
...
QStringList CategoryTreeWidget::selectedCategories() const
{
    ...
    for (int i = 0; i < topLevelItemCount(); ++i) {
        ...
        if (item->checkState(0) == Qt::Checked) {
            ...
            if (catItem->isCategory()) {
                selected.append(catItem->categoryName());
            }
        }
    }
...
void CategoryTreeWidget::setSelectedCategories(const QStringList& categories)
{
    ...
    for (int i = 0; i < topLevelItemCount(); ++i) {
        ...
        CategoryTreeItem* catItem = static_cast<CategoryTreeItem*>(item);
        if (catItem->isCategory()) {
            Qt::CheckState state = categories.contains(catItem->categoryName()) ? Qt::Checked : Qt::Unchecked;
...
// CategoryFilterPanel implementation
CategoryFilterPanel::CategoryFilterPanel(IconMetadataManager* metadataManager, QWidget *parent)
    : QFrame(parent), m_filterWidget(nullptr), m_isVisible(false) {
    m_filterWidget = new CategoryFilterWidget(metadataManager, this);
}
void CategoryFilterPanel::showPanel() { setVisible(true); m_isVisible = true; }
void CategoryFilterPanel::hidePanel() { setVisible(false); m_isVisible = false; }
void CategoryFilterPanel::onCloseButtonClicked() { hidePanel(); }
void CategoryFilterPanel::setupUI() { /* TODO */ }
void CategoryFilterPanel::setupAnimation() { /* TODO */ }
void CategoryFilterPanel::showEvent(QShowEvent *event) { QFrame::showEvent(event); }
void CategoryFilterPanel::hideEvent(QHideEvent *event) { QFrame::hideEvent(event); }
void CategoryFilterPanel::keyPressEvent(QKeyEvent *event) { QFrame::keyPressEvent(event); }
