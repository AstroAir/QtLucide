...
def test_cmake_standalone(qtlucide_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Configure
        run_command(["cmake", str(qtlucide_path), "-DCMAKE_BUILD_TYPE=Release"], 
                   cwd=build_dir)
        
        # Build library only (skip examples/tests for speed)
        run_command(["cmake", "--build", ".", "--target", "QtLucide"], 
                   cwd=build_dir)
...
def test_meson_standalone(qtlucide_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Setup
        run_command(["meson", "setup", str(build_dir), 
                    "--buildtype=release", "-Dexamples=disabled", "-Dtests=disabled"], 
                   cwd=qtlucide_path)
...
def test_xmake_standalone(qtlucide_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        run_command(["xmake", "config", "--examples=false", "--tests=false"], 
                   cwd=temp_project)
